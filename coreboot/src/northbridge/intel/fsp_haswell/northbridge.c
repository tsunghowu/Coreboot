...
*/
	/* 0xc0000 -> TSEG - FSP*/
	printk(BIOS_DEBUG, "//* 0xc0000 -> TSEG - FSP*//\n");

	base_k = 0xc0000 >> 10;
	size_k = (unsigned long)((mc_values[TSEG_REG] - FSP_RESERVE_MEMORY_SIZE) >> 10) - base_k;
	printk(BIOS_DEBUG, " base_k : %lx, size_k : %lx\n", base_k, size_k);
	ram_resource(dev, index++, base_k, size_k);	

	/* FSP -> TSEG */
	printk(BIOS_DEBUG, "//* FSP -> TSEG *//\n");
	base_k = (mc_values[TSEG_REG] - FSP_RESERVE_MEMORY_SIZE) >> 10;
	size_k = (unsigned long)(FSP_RESERVE_MEMORY_SIZE >> 10);
	printk(BIOS_DEBUG, " base_k : %lx, size_k : %lx\n", base_k, size_k);
	ram_resource(dev, index++, base_k, size_k);
...
struct chip_operations northbridge_intel_fsp_haswell_ops = {
