...
typedef struct southbridge_intel_fsp_lynxpoint_config config_t;
...
{
	struct southbridge_intel_fsp_lynxpoint_config *config = dev->chip_info;
	u32 data;
...
#endif	//#if 0 	//FSP has programmed this.
...
//<CHW+>	RCBA16(DISPBDF) = 0x0010;
//<CHW+>	RCBA32_OR(FD2, PCH_ENABLE_DBDF);
	pch_enable(dev);
}
