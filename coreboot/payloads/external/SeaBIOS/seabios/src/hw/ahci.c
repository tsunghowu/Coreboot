...
ahci_port_alloc(struct ahci_ctrl_s *ctrl, u32 pnr)
{
    ...
    dprintf(1, "AHCI: ahci_port_alloc line : %d\n", __LINE__ );
...
ahci_controller_setup(struct pci_device *pci)
{
    ...
    dprintf(1, "AHCI: cap 0x%x, ports_impl 0x%x\n",
    ...
    for (pnr = 0; pnr <= max; pnr++) {
        ...
        dprintf(1, "AHCI: ahci_port_alloc called for port : %d\n", pnr );
        if (port == NULL)
            continue;
        dprintf(1, "AHCI: ahci_port_alloc called for port : %x\n", port );
        
        run_thread(ahci_port_detect, port);
        dprintf(1, "AHCI: calling run_thread(ahci_port_detect, port); : %x\n", port );
        
    }
    dprintf(1, "AHCI: End of ahci_controller_setup\n");
}
